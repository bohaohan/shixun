...
public class ShopcarDAO {
	...
	public Shopcar findById(java.lang.Integer id) {
		...
		try {
			Shopcar instance = (Shopcar) getCurrentSession().get("Dao.Shopcar",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Shopcar instance) {
		...
		try {
			List results = getCurrentSession().createCriteria("Dao.Shopcar")
	...
	public Shopcar merge(Shopcar detachedInstance) {
		...
		try {
			Shopcar result = (Shopcar) getCurrentSession().merge(
					detachedInstance);
